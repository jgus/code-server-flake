...
# Per-version branch orchestrator for code-server-flake. See bencoding-flake/update-branches.sh for the general design.
#
# code-server-specific bits:
# - list_upstream_versions queries GitHub releases (coder/code-server). Tags are vX.Y.Z.
# - placeholder pin.nix has 2 fields (version, hashes = {...empty per-platform}).
...
list_upstream_versions() {
  gh api --paginate "/repos/coder/code-server/releases" --jq '.[].tag_name'
}
...
for v in "${tracked[@]}"; do
  ...
  if (( uv_exit != 0 )); then
    failed+=("${v}")
    echo "::warning title=Branch ${branch} skipped::update-version failed for ${v} (exit ${uv_exit}). Likely a missing platform tarball at that release; see the orchestrator log above."
